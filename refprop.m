...
%    - 'aeval': relative error(s), calibrated
%
%    SIMPLE EXAMPLES:
...
if strcmp(options.uncertainties, 'yes')
    ...
    for j = 1:size(properties, 1)
        ...
        results.(properties{j, 1}).(options.absoluteerrorqualifier) = ...
            max(2*abs(uncert_mat),[], 2);
        results.(properties{j, 1}).(options.relativeerrorqualifier) = ...
            abs(results.(properties{j, 1}).(options.absoluteerrorqualifier) / ...
